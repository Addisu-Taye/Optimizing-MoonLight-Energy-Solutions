...
plt.title("Togo - Daily Solar Irradiance & Ambient Temp")
plt.grid(True)
plt.tight_layout()
...
# Cleaning impact
df_clean.groupby("Cleaning")[["ModA", "ModB"]].mean().plot(kind="bar", title="Togo - Cleaning Impact on ModA & ModB")
plt.grid(True)
plt.tight_layout()
...
plt.title("WS vs GHI")
plt.tight_layout()
plt.savefig("figures/togo_ws_vs_ghi.png")
plt.close()

sns.scatterplot(data=df_clean, x="RH", y="Tamb")
plt.title("RH vs Tamb")
plt.tight_layout()
plt.savefig("figures/togo_rh_vs_tamb.png")
plt.close()

# Histograms
df_clean[["GHI", "WS"]].hist(bins=30, figsize=(10, 4))
plt.suptitle("Togo - GHI and Wind Speed Distribution")
plt.tight_layout()
...
plt.grid(True)
plt.tight_layout()
plt.savefig("figures/togo_bubble_chart.png")
plt.close()
